...
Chrome ve Edge'de Tampermonkey'in script çalıştırabilmesi için bir ayar açılmalıdır:

1. Tarayıcı adres çubuğuna yaz: `chrome://extensions`
2. Tampermonkey'i bul → **Detaylar** butonuna tıkla
3. **"Allow User Scripts"** seçeneğini **Açık** yap

> ⚠️ Bu adım yapılmazsa script çalışmaz.
...
Chrome and Edge require an additional setting to allow Tampermonkey to run scripts:

1. Type in the address bar: `chrome://extensions`
2. Find Tampermonkey → click **Details**
3. Enable **"Allow User Scripts"**

> ⚠️ Without this step the script will not run.
